...
import { parseCodeToAST, areNodesBasicallyEqual } from './astUtils'; // TODO: Добавить флаг ignoreTypes
import { performTextSearch } from './textSearch';
    ...
    try {
        if (isAstSupported) {
            ...
            if (!astFoundMatches) {
                ...
                const textSearchResults = await performTextSearch(document, textToFind, languageId, textToReplace, true) as { range: vscode.Range; decoration: vscode.DecorationOptions; mode: 'text'; }[];
            ...
            const textSearchResults = await performTextSearch(document, textToFind, languageId, textToReplace, false) as { range: vscode.Range; decoration: vscode.DecorationOptions; mode: 'text'; }[];
            textSearchResults.forEach(result => {
                 decorationsArray.push(result.decoration);
                 localMatchedResults.push({ range: result.range, mode: result.mode });
            });
